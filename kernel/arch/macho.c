...
uint32_t macho_spawn(const char *vfs_path, const char **argv, const char **envp) {
    ...
    pcb->regs.cs     = 0x2B;
    pcb->regs.ss     = 0x23;
    /* Inherit uid/gid from the spawning process so privilege changes propagate
     * — e.g. `do` setuid(0)'s itself, then its child must run as root.  Only
     * processes spawned directly by init (PID 1) — the GUI session and services
     * — take g_shell_uid/gid (the logged-in user, or root before login). */
    {
